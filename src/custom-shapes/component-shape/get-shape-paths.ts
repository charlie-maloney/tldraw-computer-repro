...
const pathCache = new WeakCache<IComponentShape, PathBuilder>();
export function getGeoShapePath(shape: IComponentShape) {
  return pathCache.get(shape, _getGeoPath);
}

function _getGeoPath(shape: IComponentShape) {
  ...
  switch (shape.props.geo) {
    ...
    case "check-box": {
      ...
      return new PathBuilder()
        ...
        .lineTo(clamp(ox + size * 0.82, 0, w), clamp(oy + size * 0.22, 0, h), {
          offset: 0,
        });
    }
    case "diamond":
...
function getPillPoints(width: number, height: number, numPoints: number) {
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    if (section.type === "straight") {
      points.push(
        Vec.Add(section.start, Vec.Mul(section.delta, sectionOffset))
      );
    } else {
      points.push(
        getPointOnCircle(
...
function getCloudPath(
  ...
  const bumpPoints = getPillPoints(innerWidth, innerHeight, numBumps).map(
    (p) => {
  ...
  for (let i = 0; i < wiggledPoints.length; i++) {
    ...
    const distanceBetweenWigglePoints = Vec.Dist(
      leftWigglePoint,
      rightWigglePoint
    );
    const relativeSize =
    ...
    const finalDistance =
      (Math.max(paddingX, paddingY) + curvatureOffset) * relativeSize;
    ...
    const center = centerOfCircleFromThreePoints(
      leftWigglePoint,
      rightWigglePoint,
      arcPoint
    );
